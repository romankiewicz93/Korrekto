    ...
    <div class="md:container md:mx-auto">
        ...
        <div class="flex px-5">
            <div class="w-full md:w-screen">
                <form method="POST" action="/module/{{ $modul->id }}" enctype="multipart/form-data">
                    ...
                    <div class="mb-6" style="width:66%">
                        ...
                        <input type="text" class="border border-gray-200 rounded p-2 w-full" name="kurskuerzel"
                            placeholder="Beispiel: ISEF" value="{{ $modul->kurskuerzel }}">
                        @error('kurskuerzel')
                    ...
                    m-bot15"
                            ...
                            @if ($tutors->count() > 0)
                                @foreach ($tutors as $tutor)
                                    @if ($modul->user_id == $tutor->id)
                                        <option selected value="{{ $tutor->id }}">{{ $tutor->firstName }}
                                            {{ $tutor->lastName }}</option>
                                    @else
                                        <option value="{{ $tutor->id }}">{{ $tutor->firstName }}
                                            {{ $tutor->lastName }}</option>
                                    @endif
                                @endForeach
                            @else
                    ...
                    {{-- <div class="mb-6">
                        ...
                        <img class="hidden w-48 mr-6 md:block"
                            src="{{ $modul->logo ? asset('storage/' . $modul->logo) : asset('images/no-image.png') }}"
                            alt="" />
                    </div> --}}
